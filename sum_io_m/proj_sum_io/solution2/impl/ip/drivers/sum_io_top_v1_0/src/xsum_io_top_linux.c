...
typedef struct {
    ...
    XSum_io_top_uio_map maps[ MAX_UIO_MAPS ];
} XSum_io_top_uio_info;

/***************** Variable Definitions **************************************/
static XSum_io_top_uio_info uio_info;
...
static int uio_info_read_name(XSum_io_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSum_io_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSum_io_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSum_io_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSum_io_top_Initialize(XSum_io_top *InstancePtr, const char* InstanceName) {
	XSum_io_top_uio_info *InfoPtr = &uio_info;
