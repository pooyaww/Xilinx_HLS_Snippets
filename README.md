...
15. - [ ] Enabling Temporal Data Parallelism: Host-to-Kernel Dataflow : using `clEnqueueMigrateMemObjects` (openCL1.2)instead of `clEnqueueWriteBuffer` or `clEnqueueReadBuffer` to improve the performance. Also this pragma should be added to the device code: `#pragma HLS INTERFACE ap_ctrl_chain port = return bundle = control`
